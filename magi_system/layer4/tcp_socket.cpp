...
TCPState stringToState(const std::string& stateStr) {
    if (stateStr == "CLOSED") return TCPState::CLOSED;
    if (stateStr == "LISTEN") return TCPState::LISTEN;
    if (stateStr == "SYN_SENT") return TCPState::SYN_SENT;
    if (stateStr == "SYN_RCVD") return TCPState::SYN_RCVD;
    if (stateStr == "ESTABLISHED") return TCPState::ESTABLISHED;
    if (stateStr == "FIN_WAIT_1") return TCPState::FIN_WAIT_1;
    if (stateStr == "FIN_WAIT_2") return TCPState::FIN_WAIT_2;
    if (stateStr == "CLOSING") return TCPState::CLOSING;
    if (stateStr == "TIME_WAIT") return TCPState::TIME_WAIT;
    if (stateStr == "CLOSE_WAIT") return TCPState::CLOSE_WAIT;
    if (stateStr == "LAST_ACK") return TCPState::LAST_ACK;
