...
namespace magi
{
    ...
    void TCPSegment::updateChecksum(const std::string &srcIp, const std::string &dstIp)
    {
        ...
        {
            std::cout << "[TCP] WARNING: updateChecksum called with empty IP srcIp='" << srcIp << "' dstIp='" << dstIp << "'" << std::endl;
            return;
        }
        uint16_t newChecksum = calculateChecksum(srcIp, dstIp);
        std::cout << "[TCP] updateChecksum " << srcIp << "->" << dstIp << " computed=0x" << std::hex << newChecksum << std::dec << " payloadLen=" << payload.size() << std::endl;
        checksum = newChecksum;
    }

    bool TCPSegment::validateChecksum(const std::string &srcIp, const std::string &dstIp) const
    {
        uint16_t computed = calculateChecksum(srcIp, dstIp);
        bool ok = (checksum == computed);
        std::cout << "[TCP] validateChecksum " << srcIp << "->" << dstIp << " stored=0x" << std::hex << checksum << " computed=0x" << computed << std::dec << " ok=" << ok << std::endl;
        return ok;
    }
