...
namespace magi
{
    ...
    Link::Link(std::shared_ptr<Interface> interfaceA,
               ...
               uint32_t delayMs,
               uint32_t mtu)
        : interfaceA(interfaceA), interfaceB(interfaceB), delayMs(delayMs), mtu(mtu)
    {
    }
    ...
    std::shared_ptr<Link> Link::create(std::shared_ptr<Interface> interfaceA,
                                       ...
                                       uint32_t mtu)
    {
        auto link = std::shared_ptr<Link>(new Link(interfaceA, interfaceB, delayMs, mtu));

        if (interfaceA)
            interfaceA->setLink(link);
        if (interfaceB)
            interfaceB->setLink(link);
